...
<head>
    ...
    <title>Update Place - Emotion</title>
...
$sql = "SELECT * FROM place_emotion WHERE placeemo_ID='$placeemo_ID'";
$result = mysqli_query($conn,$sql) or die ("Error in query: $sql " . mysqli_error());
$row = mysqli_fetch_array($result);
extract($row);
?>
									<form role="form" action="mapupdate_placeemo.php" method="post">
                                        <div class="form-group">
                                            <label>ID</label>
                                            <input class="form-control" name="placeemo_ID" value="<?php echo $placeemo_ID; ?>" disabled='disabled' />
											<input type="hidden" name="placeemo_ID" value="<?php echo $placeemo_ID; ?>" />
                                        </div>

										<div class="form-group">
                                            <label>Place Emotion Name</label>
                                            <input class="form-control" name="placeemo_Name" type="text" id="placeemo_Name" value="<?=$placeemo_Name;?>" size="30" placeholder="Place Emotion Name" required="required" />
											<!-- <input type="hidden" name="emoID" value="123"> -->
                                        </div>
                                        
										<div class="form-group">
                                            <label>Place EmotionType Name</label>
                                            <input class="form-control" name="placeemotype_Name" type="text" id="placeemotype_Name" value="<?=$placeemotype_Name;?>" size="30" placeholder="Place Emotion Type Name" required="required" />
                                        </div>

                                        <input type="submit" class="btn btn-default" name="update" id="update" value="Update">
                                        <input type="button" class="btn btn-default" name="button" id="button" value="Cancel" onclick="window.location='mapshow_placeemo.php'"  />
                                    </form>
                                </div>
                                <!-- /.col-lg-6 (nested) -->
